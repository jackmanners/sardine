...
withings_sleep_report_render <- function(sleep, output_file, format = c("html", "pdf"), title = "Withings Sleep Report") {
  ...
  "  # Determine the correct ID column in summary (prefer w_id, then sleep_id, then id)",
  "  id_col <- intersect(names(sleep$summary), c('w_id','sleep_id','id'))[1]",
  "  if (is.na(id_col)) id_col <- names(sleep$summary)[1]",
  "  for (i in seq_len(nrow(sleep$summary))) {",
  "    sleep_id <- sleep$summary[[id_col]][i]",
